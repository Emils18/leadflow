...
export const saveLead = async (req, res) => {
  ...
  if (!first_name || !last_name || !company_name || !email) {
    return res.status(400).json({
      error: 'First Name, Last Name, Company Name, and Email are required.',
    });
  }
  ...
  try {
    ...
    const resolvedStatusId = status_id
    ...
    const resolvedSourceId = source_id
      ? parseInt(source_id, 10)
      : (await prisma.leadSource.findFirst())?.id;

    const requestedAssignedId = assigned_user_id
      ? parseInt(assigned_user_id, 10)
      : req.user.id;

    const resolvedAssignedId = isAdmin ? requestedAssignedId : req.user.id;

    if (!resolvedStatusId || !resolvedSourceId) {
      return res.status(500).json({
        error: 'Baseline pipeline statuses or sources are absent. Seed database first.',
      });
    }
